...
<p align="center">Agen coding AI sumber terbuka (open source).</p>
<p align="center">
...
brew install anomalyco/tap/opencode # macOS dan Linux (direkomendasikan, selalu mutakhir)
...
mise use -g opencode               # OS Apa saja
...
Skrip instalasi akan memprioritaskan urutan jalur (path) instalasi berikut:

1. `$OPENCODE_INSTALL_DIR` - Direktori instalasi kustom
2. `$XDG_BIN_DIR` - Jalur yang sesuai dengan Spesifikasi Direktori Basis XDG
3. `$HOME/bin` - Direktori biner pengguna standar (jika ada atau dapat dibuat)
4. `$HOME/.opencode/bin` - Cadangan default (fallback)
...
- **plan** - Agen khusus baca (read-only) untuk analisis dan eksplorasi kode
  - Menolak pengeditan file secara default
  - Meminta izin sebelum menjalankan perintah bash
  - Ideal untuk mengeksplorasi basis kode yang belum dikenal atau merencanakan perubahan

Terdapat juga sub-agen **general** untuk pencarian kompleks dan tugas multi-langkah. Ini digunakan secara internal dan dapat dipanggil menggunakan `@general` di dalam pesan.

Pelajari lebih lanjut tentang [agen](https://opencode.ai/docs/agents).

### Dokumentasi

Untuk informasi lebih lanjut tentang cara mengonfigurasi OpenCode, [**buka dokumentasi kami**](https://opencode.ai/docs).
...
Jika Anda sedang mengerjakan proyek yang terkait dengan OpenCode dan menggunakan nama "opencode" sebagai bagian dari namanya, misalnya "opencode-dashboard" atau "opencode-mobile", harap tambahkan catatan pada README Anda untuk memperjelas bahwa proyek tersebut tidak dibuat oleh tim OpenCode dan tidak berafiliasi dengan kami dalam bentuk apa pun.
